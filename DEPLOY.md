...
本次用量优化沿用现有 KV 数据格式。已有部署直接推送并等待 Pages 自动部署即可，无需新增环境变量、重新绑定 KV、导入数据或更换任何链接、Token。节点、分享、设置、备份与历史记录保留。后续日志优化将未显式配置的部署默认改为采样，既有 `REQUESTLOG=1`/`0` 仍分别保持完整记录/关闭。
