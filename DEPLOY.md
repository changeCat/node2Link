...
3. 打开 `/api-subscriptions`，确认页面只展示一个 API Token、可重新生成，并明确展示批量 GET 和 POST 调用方式；保存一条测试模板，通过一次请求传入两个地址，确认两个节点都出现在 API 订阅中、主订阅编辑框不包含这些节点，而主订阅输出会动态附加它们。
4. 打开 `/shares`，确认分享列表正常，并确认“从已有节点选择”能同时显示主订阅和 API 订阅节点。
5. 点击一个分享的“修改”，确认节点内容能在点击后加载。
6. 创建一个临时分享，复制链接并请求一次订阅。
