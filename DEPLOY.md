...
本次 v3 存储重构不改变现有部署设置：继续使用 `npm run pages:build`、输出 `dist` 和原 `KV` 绑定及环境变量。

**本次需要重新录入数据。** 为消除历史扫描，v3 不再读取旧主订阅、设置、分享或 API 节点，也不自动迁移。升级前备份，部署后重新添加；旧记录不会删除。固定身份、已保存的 API 模板/Token 与请求日志仍直接读取。回滚注意事项见 [架构与存储](ARCHITECTURE.md#校验与回滚)。
...
重构不要求新建 KV，也不会修改 KV Namespace ID、域名或环境变量。旧分享需要重新创建并更新客户端地址。
...
升级后先从备份重新保存主订阅、设置和 API 节点，再创建分享。旧格式分享不会自动显示；新建分享需要重新分发链接。

### KV 用量优化版本

v3 设置与分享详情按固定键读取，主订阅通过固定 head 读取版本；API 节点关闭时正常订阅为零 List。API 节点的并发批次日志仍需要扫描，管理列表和请求统计也会使用 List。日志默认采样 10%，既有 `REQUESTLOG=1`/`0` 仍保持完整记录/关闭；页面设置需重新保存。

公开订阅的显示/转换设置增加最多 60 秒的实例内缓存，管理端分享摘要缓存为 15 秒。正文和分享访问权限校验不使用这两类缓存。设置保存会使当前实例缓存失效，其他实例仍受上述有效期和 KV 自身传播延迟影响。
...
构建部署不会清空 KV。回滚到 v2 会重新读取旧数据而看不到 v3 修改，并可能重新启用只在 v3 撤销的链接；回滚前检查两套数据及权限，详见 `ARCHITECTURE.md`。不要删除版本正文或撤销标记来回滚。

本次不改变会话签名方式。GitHub Verify 运行桌面/手机浏览器回归测试；Cloudflare Pages 构建命令仍为 `npm run pages:build`，无需安装 Chromium 或新增绑定。
