...
async function serveSubscription(request, env, ctx, runtime, sourceData, access, includeWarp, subscriptionId = '', subscriptionName = '') {
		...
		if (!isSubConverterRequest && request.method === 'GET' && shouldSendSubscriptionNotification(request)) {
			const subscriptionType = access === 'share' ? '分享订阅' : '主订阅';
			const subscriptionIdentity = access === 'share' && subscriptionId
				? `\n订阅 ID: <code>${escapeHTML(subscriptionId)}</code>`
				: '';
			queueTelegram(ctx, sendMessage(runtime, `#获取订阅\n订阅类型: ${subscriptionType}\n订阅名称: ${escapeHTML(effectiveSubscriptionName)}${subscriptionIdentity}`, request.headers.get('CF-Connecting-IP'), `UA: ${userAgentHeader}</tg-spoiler>\n域名: ${url.hostname}\n<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`));
		}
