...
async function handleSharesAPI(request, env) {
	...
	try {
		...
		if (request.method === 'PATCH') {
			...
			const newIndex = index.includes(id)
				? index.map(item => item === id ? newId : item)
				: [newId, ...index];
			await env.KV.put(SHARE_KEY_PREFIX + newId, JSON.stringify(share));
			await env.KV.put(SHARE_INDEX_KEY, JSON.stringify(newIndex));
...
async function renderSharesPage(_request, env, runtime) {
	...
	return new Response(html.replace('</body>', resetLinkScript + '</body>'), { headers: { 'Content-Type': 'text/html;charset=utf-8', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff' } });
}
