...
test('conversion settings manage two types with one active profile and preserve inactive entries', async ({ page }) => {
 await page.goto('/settings');
 await expect(page.locator('#conversionForm')).toContainText('回退默认');
 ...
 const first = page.locator('.converter-profile').nth(0);
 await first.locator('[data-field="name"]').fill('VPS 转换');
 await first.locator('[data-field="url"]').fill('https://custom.example.com/browser_test_key');
 await page.locator('#addConverter').click();
 const second = page.locator('.converter-profile').nth(1);
 await second.locator('[data-field="name"]').fill('Worker 转换');
 await second.locator('[data-field="type"]').selectOption('sublink');
 await second.locator('[data-field="url"]').fill('https://worker.example.com');
 await second.locator('input[type="radio"]').check();
 await expect(page.locator('input[name="activeCustomConverter"]:checked')).toHaveCount(1);
 ...
 await expect(first.locator('[data-field="url"]')).toHaveValue('https://custom.example.com/browser_test_key');
 await expect(second.locator('input[type="radio"]')).toBeChecked();
 await expect(second.locator('[data-field="type"]')).toHaveValue('sublink');
 await first.locator('input[type="radio"]').check();
 await expect(page.locator('#activeConverterMode')).toHaveText('自建 Subconverter');
 await page.locator('#conversionForm button[type="submit"]').click();
 await expect(page.locator('#conversionMessage')).toHaveText('已保存');
 await page.goto('/');
 await expect(page.locator('.converter-list')).toContainText('custom.example.com/browser_test_key');
 await expect(page.locator('.converter-list')).not.toContainText('worker.example.com');
 await expect(page.locator('.converter-warning')).toContainText('自定义不可用时回退默认');
 await page.goto('/settings');
 ...
 await second.locator('[data-field="name"]').fill('保留未保存内容');
 await page.locator('#conversionForm button[type="submit"]').click();
 await expect(page.locator('#conversionMessage')).toHaveText('保存失败测试');
 await expect(second.locator('[data-field="name"]')).toHaveValue('保留未保存内容');
 await page.unroute('**/api/settings');
 await first.locator('[data-remove]').click();
 await expect(page.locator('.converter-profile')).toHaveCount(1);
 await expect(page.locator('input[name="activeCustomConverter"]')).toBeChecked();
 await page.locator('input[name="converterMode"][value="default"]').check();
 await page.locator('#conversionForm button[type="submit"]').click();
 await expect(page.locator('#conversionMessage')).toHaveText('已保存');
 await page.reload();
