...
风险优先策略下，Loon 继续先由 Worker 完整读取普通远程上游，再把合并结果交给转换器，并对已经读取的节点执行严格完整性校验；不会为了提速而把远程节点数量降级为“未完全核验”。Loon 遇到无法在本地准确计数的 Clash/Mihomo YAML、Sing-box JSON 或远程 WARP 来源时会停止更新并返回 502，既不把原始来源 URL/Token 交给转换器，也不返回可能缺少节点的新订阅。其他转换格式可使用两分钟内有效、经过认证加密的本站代理地址，转换器看不到原始订阅 URL 或其中的 Token。代理仅在远程来源不超过 8 个且组合地址处于安全长度内时启用；加密失败、来源过多或地址过长都会自动回到原有的预读取流程，不会直接导致订阅失败。
