...
- 节点检查、去重、优选草稿、TXT 导入导出与原始节点历史版本恢复；
...
主订阅以紧凑卡片展示原始节点的名称、协议和地址，可查看完整链接或逐条编辑。节点支持多选、全选筛选结果与批量删除，选择包含尚未展开的匹配节点，删除会清理关联且可撤销。三块节点/地址列表限制高度并独立滚动，顶部保留状态与数量，管理操作集中在原始节点区。通过批量添加弹窗追加或覆盖节点，两种操作均拒绝空白输入；覆盖前确认移除的节点及关联，完全相同的链接保留 ID，其余旧节点关联被清理，无关联的地址停用。优选地址支持逐行添加，每行独立填写域名或 IP、端口和备注，再勾选要应用的原始节点。地址字段不填写端口，IPv6 可直接填写。扩展名称使用“原节点名-备注”，备注为空时使用地址和端口。端口下拉包含 Cloudflare 全部官方 HTTP / HTTPS 端口，也可选择自定义端口。

原始节点的追加、覆盖、编辑、删除、去重和 TXT 导入立即保存并更新订阅，已生效扩展节点同步采用最新的原始节点参数。优选地址与关联在独立区域编辑，点击该区域顶部“保存全部并生效”后发布。保存原始节点不会发布尚未保存的优选草稿。原始节点和优选地址支持拖动或用手柄上的方向键排序：原始节点排序立即保存，优选地址排序随“保存全部并生效”发布。预览和订阅输出均按原始节点顺序排列，每个原始节点后紧跟按优选地址顺序生成的扩展节点，API 节点按添加顺序追加到末尾。生成时仅修改连接地址、端口和节点名称，保留凭据、Host、SNI、路径及其他参数；协议是否适用于优选由管理员自行确认，不做协议适用性判断或测速。VMess JSON、SSR 和 Shadowsocks 编码链接会按格式解码修改，无法解析的扩展会阻止本次保存并标明相关节点。

从原始节点列表逐条编辑可以保留关联 ID。修改名称、参数或重置 UUID 请使用逐条编辑，关联不变；批量覆盖不同链接不会猜测其身份；删除节点会移除其关联，并停用没有剩余关联的地址。停用地址保留配置，保存后停止输出其扩展。历史手工扩展节点按原始节点保留，不自动识别或拆分。

原始节点历史版本默认保留最近 3 个（包含最新版本），可在设置中调整为 1–20 个。仅原始节点变化记录新版本，优选配置保存不占名额；可选择版本查看完整链接、下载 TXT 或立即还原。还原保留当前优选配置并清理失效关联，不恢复已删除的关联。减少保留数量后，超出范围的版本不再显示，下次保存时清理。TXT 导入覆盖原始节点，相同链接保留关联；TXT 可分别导出原始节点（含订阅源）、扩展节点或完整预览，不受搜索筛选影响。优选草稿保存在本地。最多配置 1000 条优选地址、生成 10000 个扩展，配置与输出各限 20 MiB，整个版本序列化后限 24 MiB。
...
遇到主订阅编辑冲突时，弹窗输入和优选草稿会保留。先复制弹窗输入或导出当前预览，再刷新读取最新内容并合并修改。此检查基于 D1 head 的当前版本，适用于个人多标签页操作；正文仍通过不可变 KV 版本发布。
...
风险优先策略下，Loon 继续先由 Worker 完整读取普通远程上游，再把合并结果交给转换器，并对已经读取的节点执行严格完整性校验；不会为了提速而把远程节点数量降级为“未完全核验”。Loon 遇到无法在本地准确计数的 Clash/Mihomo YAML、Sing-box JSON 或远程 WARP 来源时会停止更新并返回 502，既不把原始来源 URL/Token 交给转换器，也不返回可能缺少节点的新订阅。其他转换格式可使用两分钟内有效、经过认证加密的本站代理地址，转换器看不到原始订阅 URL 或其中的 Token。代理仅在远程来源不超过 8 个且组合地址处于安全长度内时启用；普通来源可安全回到预读取流程，结构化来源或远程 WARP 若无法加密隐藏则停止更新，不会退回明文传递原始 URL。
...
速度与隐私：普通上游并发读取（默认最多 6 个），共享 8 秒完整响应预算；遇到网络瞬断、429 或 5xx 时可在这 8 秒内立即重试一次，404 等确定性错误不重试，也不会延长总等待时间。Loon 使用严格预读取；其他转换格式在满足防护条件时通过短时加密代理读取。转换阶段自定义与默认共享 30 秒总预算，自定义单次最多 8 秒，剩余时间供默认使用；默认列表多个后端时单次最多 20 秒。收到失败状态立即取消错误响应体，不等待错误页面下载。不并发向自定义和默认发送节点，不新增节点缓存或额外持久化副本。转换回调明确标记为来源读取，避免 Sublink 再次触发转换。转换请求不强制覆盖节点的 TLS 证书校验与 TCP Fast Open 参数。
