...
主订阅以紧凑卡片展示原始节点的名称、协议和地址，可查看完整链接或逐条编辑。通过批量添加弹窗追加或覆盖节点；覆盖前确认移除的节点及关联，完全相同的链接保留 ID，其余旧节点关联被清理，无关联的地址停用。优选地址支持逐行添加，每行独立填写域名或 IP、端口和备注，再勾选要应用的原始节点。地址字段不填写端口，IPv6 可直接填写。扩展名称使用“原节点名-备注”，备注为空时使用地址和端口。Cloudflare 常用端口提供输入建议，也可自行填写其他合法端口。
