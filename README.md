...
- 独立设置页，可分别修改主订阅名称、标签页标题与图标、主订阅入口 Token、转换配置。自定义转换支持 Sublink Worker 与 Subconverter，最多保存 10 条，只启用选中的一条；该项不可用时回退默认，不尝试其他未启用项。无需转换的通用节点订阅本地生成；还可增减及排序“我的订阅”所展示的客户端格式，转换配置逐项自动保存，其余模块独立保存。
...
转换服务提供默认服务与自定义服务。自定义配置包含名称、类型和基础地址，最多 10 条，通过按钮弹窗添加和编辑，列表单选指定唯一生效项；添加、编辑、切换模式、启用和删除后直接保存。保存成功才更新列表；失败保留弹窗内容或恢复原来的启用状态。删除最后一条时自动切回默认服务。选择默认模式时仍保留自定义列表。现有单地址配置自动显示为一条 Subconverter 配置，历史明确保存了 Sublink 类型的配置保留该类型。地址可包含访问密钥路径；通知和诊断响应头仅显示域名，不显示密钥路径。
