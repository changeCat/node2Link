...
原始节点的追加、覆盖、编辑、删除、去重和 TXT 导入立即保存并更新订阅，已生效扩展节点同步采用最新的原始节点参数。优选地址与关联在独立区域编辑，点击该区域顶部“保存全部并生效”后发布。保存原始节点不会发布尚未保存的优选草稿。生成时仅修改连接地址、端口和节点名称，保留凭据、Host、SNI、路径及其他参数；协议是否适用于优选由管理员自行确认，不做协议适用性判断或测速。VMess JSON、SSR 和 Shadowsocks 编码链接会按格式解码修改，无法解析的扩展会阻止本次保存并标明相关节点。
