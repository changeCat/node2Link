...
- 独立设置页，可分别修改客户端订阅名称、浏览器标签页标题与图标、主订阅入口 Token，并在默认/自建转换服务和默认/自建规则之间切换；还可增减及排序“我的订阅”所展示的客户端格式，各设置模块均可独立保存，标签页标题默认使用 `CF-Workers-SUB`；
- “主订阅”“分享管理”和“订阅请求”使用顶部 Tab 切换，访问根路径时默认展示主订阅；
- 独立分享管理页，可为不同节点组生成不同订阅链接，并支持重复创建、修改和删除；分享订阅与主订阅使用相同的客户端自适应转换逻辑；
- 主订阅与分享订阅统一使用 `/s/<随机ID>`，管理密码不会出现在订阅地址中；
- 节点检查、去重、草稿、备份与最近一次版本恢复；
- 独立的近 30 天订阅请求统计页，主订阅与各分享订阅分别记录和展示；
...
6. 在“订阅请求”中分别查看主订阅及每个分享订阅近 30 天的请求记录；
