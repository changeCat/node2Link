...
- 独立设置页，可分别修改主订阅名称、标签页标题与图标、主订阅入口 Token、转换及规则配置。自定义转换支持 Sublink Worker 与 Subconverter，最多保存 10 条，只启用选中的一条；该项不可用时回退默认，不尝试其他未启用项。无需转换的通用节点订阅本地生成；还可增减及排序“我的订阅”所展示的客户端格式，各模块独立保存。
...
转换服务提供默认服务与自定义服务。自定义配置包含名称、类型和基础地址，最多 10 条，用单选指定唯一生效项；选择默认模式时仍保留自定义列表。现有单地址配置自动显示为一条 Subconverter 配置，历史明确保存了 Sublink 类型的配置保留该类型。地址可包含访问密钥路径；通知和诊断响应头仅显示域名，不显示密钥路径。

Subconverter 使用 `/sub?target=...&url=...&config=...`；Sublink Worker 使用 `/clash`、`/singbox`、`/surge`、`/xray`，以 `config` 参数传递换行分隔的来源链接。Sublink 使用自身规则，设置页的 INI 规则仅供 Subconverter（含默认回退）使用。Sublink 的 Loon / QuanX 接口不受支持，因此不发送无效请求，直接回退默认并通知原因。
