...
模块划分和 D1/KV 一致性边界见 [架构与存储](ARCHITECTURE.md)。Pages Git 自动部署方式和现有环境变量不变；运行时必须同时绑定 D1 `DB` 和 KV `KV`。

**全新存储配置：** 当前版本要求同时绑定空 D1 `DB` 和空 KV `KV`，不包含历史 KV 格式的迁移或回退。首次部署前先在 D1 控制台手工执行 `migrations/0001_storage.sql`，随后登录并添加数据。

## 功能

- 账号密码登录管理端，会话 Cookie 使用 `HttpOnly`、`Secure`、`SameSite=Strict`；
- 汇聚多个节点或上游订阅，并输出 Base64、Clash、Sing-box、Surge、QuanX、Loon 等格式；
- 独立设置页，可分别修改主订阅名称、标签页标题与图标、主订阅入口 Token、转换配置。自定义转换支持 Sublink Worker 与 Subconverter，最多保存 10 条，只启用选中的一条；该项不可用时回退默认，不尝试其他未启用项。无需转换的通用节点订阅本地生成；还可增减及排序“我的订阅”所展示的客户端格式，服务列表增删改自动保存，转换模式与启用项点击“保存”后生效，其余模块独立保存。
...
主订阅页面分为原始节点和优选地址两个编辑区，下面展示生成结果。批量粘贴原始节点后，添加优选域名或 IP、固定端口与备注，再勾选要应用的原始节点；支持一行一个地址批量添加，行内端口优先于默认端口，IPv6 携带端口时使用方括号。Cloudflare 常用端口提供输入建议，也可自行填写其他合法端口。

点击“保存并生效”后，订阅返回原始节点及对应扩展节点。生成时仅修改连接地址、端口和节点名称，保留凭据、Host、SNI、路径及其他参数；协议是否适用于优选由管理员自行确认，不做协议适用性判断或测速。VMess JSON、SSR 和 Shadowsocks 编码链接会按格式解码修改，无法解析的扩展会阻止本次保存并标明相关节点。

从原始节点列表逐条编辑可以保留关联 ID。批量替换不同链接不会猜测其身份，如旧链接仍被关联，需要重新选择关联；删除节点会移除其关联，并停用没有剩余关联的地址。停用地址保留配置，保存后停止输出其扩展。历史手工扩展节点按原始节点保留，不自动识别或拆分。

JSON 备份、本地草稿和上次版本包含完整原始节点与关联配置；TXT 用于导出生成后的节点，导入 TXT 会作为原始节点载入并清空优选配置。生成结果及配置属于同一保存版本。最多配置 1000 条优选地址、生成 10000 个扩展，配置与输出各限 20 MiB，整个版本序列化后限 24 MiB。

分享选择器按原始节点分组，支持筛选主订阅原始/扩展节点与选择整组；每次打开重新加载已保存结果。分享保存选中链接的快照，之后修改主订阅不会自动更新已有分享。API 订阅的模板、存储、导入和动态附加规则保持不变。此次升级无需新增数据库表。

## 部署前配置

必须同时配置 Cloudflare KV 与 D1：KV 命名空间变量名保持为 `KV`，D1 数据库绑定变量名固定为 `DB`。D1 保存结构化状态和常规分享正文，KV 保存 20 MiB 主订阅正文以及 JSON 转义后接近 D1 行上限的极端分享正文。未绑卡的 Workers Free 账号也可创建 D1；免费额度见 [官方计费](https://developers.cloudflare.com/d1/platform/pricing/) 和 [限制](https://developers.cloudflare.com/d1/platform/limits/)。
...
| `LINK` | 否 | `vless://...` | 尚未在页面保存主订阅时使用的初始只读节点来源 |
...
订阅请求固定完整记录，保留 30 天；统计页最多读取最近 500 条，超过时会提示仅展示最近部分。按每天约 50 次订阅请求计算，这部分 D1 写入远低于免费额度。
...
页面随机生成的主订阅 Token 与新建分享的随机 ID 均为 32 位 Base64URL 字符串。环境变量 `TOKEN` 入口保持可用；页面保存的新 Token 会覆盖该默认值。
...
在 Pages 项目的“设置 → 绑定”中保留变量名为 `KV` 的 KV 命名空间，再创建或选择一个 D1 数据库并以 `DB` 绑定。绑定变化后重新部署一次；首次部署前在 D1 控制台手工执行 `migrations/0001_storage.sql`。然后在“变量和机密”中保留 `ADMIN_PASSWORD`、`SESSION_SECRET` 等原配置。
...
完整的全新配置、自动部署和验证步骤见 [Cloudflare Pages 部署手册](DEPLOY.md)。
...
分享列表从 D1 metadata 读取摘要，常规分享详情直接按主键从 D1 读取；仅接近 D1 行上限的极端正文再按指针从 KV 精确读取。普通订阅不执行 KV List。API 节点由 D1 范围查询一次取回，统计最多读取最近 500 条请求事件。管理页面响应包含 `Server-Timing`，可在浏览器开发者工具中查看 Worker 总处理时间。

开发验证可运行 `npm run pages:build`。安装浏览器（`npx playwright install chromium`）后，运行 `npm run test:e2e` 检查桌面和手机交互；`npm run benchmark` 查看本地 D1/KV 操作数。浏览器测试由 GitHub Actions 单独执行，不改变 Pages 的构建命令。
...
8. 分享内容修改后原链接保持不变；删除和重置由 D1 事务状态控制，成功响应后旧链接失效。
...
登录后从顶部“仪表盘”进入 `/dashboard`。节点数量按已保存的主订阅与 API 节点内容合并去重，不含上游远程节点和分享副本，不表示节点在线率；上游来源数量只统计主订阅内保存的链接。到期提醒包含暂停的分享，最多列出最近 8 项；最近修改展示主订阅、现存分享和 API 节点的最新时间，并非完整操作日志。页面按需加载，可点击“刷新概览”更新，不轮询，也不主动拉取上游。

遇到主订阅编辑冲突时，当前文本和本地草稿会保留。先点击“备份”下载当前编辑，再刷新读取最新内容并合并修改。此检查基于 D1 head 的当前版本，适用于个人多标签页操作；正文仍通过不可变 KV 版本发布。

转换服务提供默认服务与自定义服务。自定义配置包含名称、类型和基础地址，最多 10 条，通过按钮弹窗添加和编辑，列表单选指定唯一生效项；添加、编辑和删除直接保存到列表；默认/自定义模式与启用项的选择必须点击“保存”才生效。保存服务信息不会顺带提交尚未保存的转换选择。保存失败保留弹窗输入或待保存选择，当前生效服务保持原值。删除正在生效的服务时回到默认，不自动启用其他自定义项。选择默认模式时仍保留自定义列表。现有单地址配置自动显示为一条 Subconverter 配置，历史明确保存了 Sublink 类型的配置保留该类型。地址可包含访问密钥路径；通知和诊断响应头仅显示域名，不显示密钥路径。

Subconverter 使用 `/sub?target=...&url=...&config=...`；Sublink Worker 使用 `/clash`、`/singbox`、`/surge`、`/xray`、`/loon`、`/quanx`，以 `config` 参数传递换行分隔的来源链接。Subconverter（含默认回退）固定使用代码中的内置默认规则，移除设置页与 API 的自定义规则入口，旧规则设置和 SUBCONFIG 环境变量不再生效。Sublink 使用自身规则。所有目标格式均实际尝试自定义接口，不按固定支持列表跳过；HTTP 失败、无效格式或节点校验失败时回退默认。后续 Sublink 若沿用相同路径、参数和输出格式新增支持，无需修改本项目即可使用。

所有客户端按目标格式决定是否转换：原始节点、明文或 Base64 上游可以在本站合并编码为 Base64；Loon 等专用格式仍需转换。Clash/Mihomo YAML、Sing-box JSON 等结构化上游交给转换后端处理。需要转换时先请求唯一生效的自定义项；网络错误、超时、空内容、格式无效或 Loon 已知节点丢失会触发默认回退。默认列表按顺序尝试并使用相同的响应校验，均不可用则返回 502。上游来源读取失败时也停止更新，避免使用不完整节点覆盖客户端订阅。

Loon 按协议比较已经读取的节点数量与转换输出。14 个 VLESS 与 4 个 HY2 只输出 4 个 HY2 时，该结果会被拒绝并触发回退；默认也丢节点时返回 502。远程结构化来源、WARP 或输出 Remote Proxy 无法完整计数时，通知明确标注未完全核验。数量检查不能证明每个参数无损或节点可连接。官方 `tindy2013/subconverter` 镜像继续可用，但当前确认的 a0d4eab 版本不含 VLESS；此类转换依赖默认回退，见 [协议模型](https://github.com/tindy2013/subconverter/blob/a0d4eab/src/parser/config/proxy.h)。

速度与隐私：普通上游并发读取（默认最多 6 个），共享 8 秒完整响应预算。转换阶段自定义与默认共享 30 秒预算，自定义单次最多 8 秒，剩余时间供默认使用；默认列表多个后端时单次最多 20 秒。收到失败状态立即取消错误响应体，不等待错误页面下载。不并发向自定义和默认发送节点，不新增节点缓存或额外持久化副本。转换回调明确标记为来源读取，避免 Sublink 再次触发转换。转换请求不强制覆盖节点的 TLS 证书校验与 TCP Fast Open 参数。

主订阅页面重点标注未启用自定义的情况，并提示自定义失败后的默认回退。通知显示目标格式、实际转换服务、回退原因及结果；本地生成时显示“无需转换”和当前配置。回退默认意味着默认服务可以读取订阅来源和节点信息。

v2rayN 获取 Base64 订阅时，项目保留既有兼容处理：为只有 `peer` 的 AnyTLS 节点补充同值 `sni`，过滤当前 v2rayN 无法解析的 `obfs-local;obfs=tls` Shadowsocks 节点，并在响应头说明。其他客户端不应用这些 v2rayN 专用调整。
...
全新部署必须同时绑定 D1 `DB` 与 KV `KV`；绑定后必须先手工执行 `migrations/0001_storage.sql`；代码不会创建或修改表结构。完成后可从登录页面添加数据，现有环境变量名称和含义保持不变。
...
- 设置、身份、分享、API 节点、API 配置、请求日志及主订阅 head 保存到 D1；主订阅正文和超出 D1 安全行大小的分享正文保存到 KV；完整键布局见架构文档；
...
- 删除分享会在 D1 发布永久撤销标记，再尽力清理 KV 正文；撤销成功后无法从管理页恢复；
- 修改 `ADMIN_PASSWORD` 或 `SESSION_SECRET` 会使使用新配置的实例拒绝旧登录会话，但不会改变订阅链接；会话签名升级后管理员需重新登录一次；
- 主订阅用 D1 固定 head 指向 KV 独立正文版本，保留当前及最近一次保存版本和 20 MiB 上限；
