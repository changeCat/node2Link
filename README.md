...
JSON 备份、本地草稿和上次版本包含完整原始节点与关联配置；TXT 可分别导出全部原始节点（含订阅源）、扩展节点或完整结果，不受当前搜索和预览筛选影响；JSON 用于完整恢复节点 ID 和优选关联。导入 TXT 会作为原始节点载入并清空优选配置。生成结果及配置属于同一保存版本。最多配置 1000 条优选地址、生成 10000 个扩展，配置与输出各限 20 MiB，整个版本序列化后限 24 MiB。
