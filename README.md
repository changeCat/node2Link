...
模块划分、D1/KV 一致性边界及回滚说明见 [架构与存储](ARCHITECTURE.md)。Pages Git 自动部署方式和现有环境变量不变；本版新增一个名为 `DB` 的 D1 绑定，原 `KV` 绑定继续保留。

**D1 切换需要重新录入数据：** 本版不把旧 KV 结构化记录迁入 D1。绑定 `DB` 并重新部署后，以新的空 D1 为准；请先备份，再重新保存主订阅、设置、API 节点并创建分享。旧 KV 键不会删除，也不会进入在线扫描路径。
...
页面随机生成的主订阅 Token 与新建分享的随机 ID 均为 32 位 Base64URL 字符串。D1 不自动读取旧 KV 分享，重新创建后需更新客户端链接；环境变量 TOKEN 入口保持可用，页面保存的 Token 需重新配置。
...
升级已有部署需保留原 KV 并新增一次 D1 `DB` 绑定，不需要修改现有环境变量。D1 生效后不读取旧 KV 结构化记录，需要从备份重新保存主订阅、设置、API 节点并创建分享。
...
- 主订阅用 D1 固定 head 指向 KV 独立正文版本，保留当前及最近一次保存版本和 20 MiB 上限；不回读旧 `LINK.txt` 与 `LINK.backup.txt`；
