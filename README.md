...
**全新存储配置：** 当前版本要求同时绑定空 D1 `DB` 和空 KV `KV`，不包含历史 KV 格式的迁移或回退。首次部署后直接登录并添加数据，D1 表会自动创建。
...
在 Pages 项目的“设置 → 绑定”中保留变量名为 `KV` 的 KV 命名空间，再创建或选择一个 D1 数据库并以 `DB` 绑定。绑定变化后重新部署一次；表结构会在第一次请求时自动初始化。然后在“变量和机密”中保留 `ADMIN_PASSWORD`、`SESSION_SECRET` 等原配置。
...
全新部署必须同时绑定 D1 `DB` 与 KV `KV`；绑定后无需手工建表，登录页面直接添加数据即可。现有环境变量名称和含义保持不变。
