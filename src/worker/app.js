...
function renderSettingsPage(_request, runtime) {
	...
	</style></head><body>${renderTopbar('settings', runtime)}<main><div class="page-head"><h1>设置</h1><p>四个模块可以分别保存，未提交的其他模块不会被修改。</p></div><div class="settings-form">
...
async function renderGeneratedNodesPage(_request, env, runtime) {
	...
	const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>API 订阅 · ${escapeHTML(runtime.pageTitle)}</title>${renderFavicon(runtime.browserIconURL)}<style>${basePageStyles()}
		...
		document.querySelector('[data-build-node-url]').addEventListener('click',function(){var button=this;var node=window.prompt('粘贴一个完整节点，例如 vless://...');if(node===null)return;node=node.trim();if(!/^(vless|vmess|trojan|ss|ssr|hysteria|hysteria2|hy2|tuic|wireguard|socks|socks5):\\/\\//i.test(node)){document.getElementById('settingsMessage').textContent='请输入支持的完整节点链接';document.getElementById('settingsMessage').className='message';return}var url=window.location.origin+'/api/import?token='+encodeURIComponent(tokenInput.value)+'&node='+encodeURIComponent(node);document.getElementById('nodeExample').textContent=url;copyText(url).then(function(){button.textContent='已自动编码并复制';setTimeout(function(){button.textContent='粘贴并复制'},1400)})});
