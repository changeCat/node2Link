...
async function renderGeneratedNodesPage(_request, env, runtime) {
	...
	const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>API 订阅 · ${escapeHTML(runtime.pageTitle)}</title>${renderFavicon(runtime.browserIconURL)}<style>${basePageStyles()}
		.api-layout{display:grid;grid-template-columns:minmax(420px,.9fr) minmax(460px,1.1fr);gap:18px;align-items:start}.config-panel h2,.nodes-panel h2{margin:0;font-size:19px}.section-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px}.section-head p{margin:5px 0 0;color:var(--muted);font-size:13px}.count-badge{flex:0 0 auto;padding:5px 10px;border-radius:999px;background:var(--green-soft);color:var(--green-dark);font-size:12px;font-weight:800}.token-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}.token-row input{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}.template-editor{min-height:176px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:12px;line-height:1.65}.placeholder-help{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.placeholder-help code{padding:3px 6px;border-radius:5px;background:#f1f5f1;color:var(--green-dark);font-size:11px}.variable-guide{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:9px}.variable-guide div{min-width:0;padding:7px 8px;border-radius:6px;background:#f7f9f7;color:var(--muted);font-size:10px;line-height:1.45}.variable-guide code{display:block;margin-bottom:2px;color:var(--green-dark);font-size:10px;font-weight:800}.api-examples{display:grid;gap:9px;margin-top:18px;padding-top:18px;border-top:1px solid var(--line-soft)}.example-heading{margin-top:8px}.example-box{display:grid;gap:5px;padding:9px 10px;border:1px solid var(--line);border-radius:7px;background:#f8faf8}.example-box b{font-size:11px}.example-box code{overflow:hidden;color:#46544c;font-size:10px;line-height:1.55;text-overflow:ellipsis;white-space:nowrap}.method-badge{padding:3px 7px;border-radius:5px;background:var(--green);color:#fff;font-size:10px;font-weight:800}.api-call-row{min-width:0;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px;padding:7px 8px;border:1px solid var(--line);border-radius:7px;background:#f8faf8}.api-call-row code{min-width:0;overflow:hidden;color:#46544c;font-size:10px;text-overflow:ellipsis;white-space:nowrap}.api-call-row .button{min-height:32px;white-space:nowrap}.node-list{display:grid;max-height:min(72vh,760px);gap:9px;overflow-y:auto;overscroll-behavior:contain;padding-right:4px;scrollbar-gutter:stable}.node-card{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px;border:1px solid var(--line);border-radius:8px;background:#fff}.node-main{min-width:0}.node-head{display:flex;align-items:center;gap:8px}.node-head strong{min-width:0;overflow:hidden;font-size:13px;text-overflow:ellipsis;white-space:nowrap}.node-kind{flex:0 0 auto;padding:2px 6px;border-radius:999px;background:var(--green-soft);color:var(--green-dark);font-size:9px;font-weight:800}.node-meta{margin-top:4px;color:var(--muted);font-size:10px}.node-content{margin-top:6px;overflow:hidden;color:#56615b;font:10px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;text-overflow:ellipsis;white-space:nowrap}.node-actions{display:flex;gap:7px}.empty-list{padding:44px 18px;border:1px dashed var(--line);border-radius:8px;color:var(--muted);text-align:center}.save-row{display:flex;align-items:center;gap:10px;margin-top:18px}.danger-button{border-color:#f2c6c2;color:var(--danger)}.confirm-dialog{width:min(420px,calc(100% - 32px));padding:0;border:0;border-radius:14px;box-shadow:0 28px 90px rgba(12,35,24,.28)}.confirm-dialog::backdrop{background:rgba(12,28,20,.45);backdrop-filter:blur(2px)}.confirm-body{padding:24px}.confirm-body h3{margin:0 0 8px}.confirm-body p{margin:0;color:var(--muted);line-height:1.65}.confirm-actions{display:flex;justify-content:flex-end;gap:9px;padding:0 24px 20px}@media(max-width:980px){.api-layout{grid-template-columns:1fr}.node-list{max-height:none}}@media(max-width:580px){.token-row{grid-template-columns:1fr}.variable-guide{grid-template-columns:1fr}.api-call-row{grid-template-columns:auto minmax(0,1fr) auto}.api-call-row .button{padding:0 8px;font-size:10px}.node-card{grid-template-columns:1fr}.node-actions{justify-self:end}}
	</style></head><body>${renderTopbar('api-subscriptions')}<main><div class="page-head"><h1>API 订阅</h1><p>通过统一地址参数或完整节点链接追加节点；节点在本页独立维护，并自动附加到主订阅结果末尾。</p></div><div class="api-layout"><section class="panel config-panel"><div class="section-head"><div><h2>模板与 API 凭证</h2><p>本页仅保留一个有效 Token；先保存模板，再把调用地址交给第三方。</p></div></div><form id="settingsForm"><div class="field"><label for="apiToken">唯一 API Token</label><div class="token-row"><input id="apiToken" type="text" maxlength="128" readonly><button class="button" id="regenerateToken" type="button">重新生成 Token</button></div><small>首次打开本页时自动生成一个 Token；重新生成并保存后，旧 Token 和旧调用 URL 立即失效。</small></div><div class="field"><label for="nameTemplate">节点名称格式</label><input id="nameTemplate" type="text" maxlength="160" placeholder="CF-{{type}}-{{address|split:.:0}}:{{port}}" required><small>支持 <code>|slice:开始:结束</code> 和 <code>|split:分隔符:序号</code>，例如 <code>{{address|split:.:0}}</code>。</small></div><div class="field"><label for="nodeTemplate">节点模板</label><textarea class="template-editor" id="nodeTemplate" placeholder="vless://uuid@{{address}}:{{port}}?security=tls#{{name}}" spellcheck="false" required></textarea><small>每行一个模板；一次地址调用会为每行生成一个节点。</small><div class="placeholder-help"><code>{{address}}</code><code>{{port}}</code><code>{{name}}</code><code>{{type}}</code></div><div class="variable-guide"><div><code>{{address}}</code>域名、IPv4 或 IPv6；节点模板中的 IPv6 会自动加方括号。</div><div><code>{{port}}</code>API 传入的端口；省略时随机使用 Cloudflare HTTPS 端口。</div><div><code>{{name}}</code>由名称格式生成并完成 URL 编码的节点名称。</div><div><code>{{type}}</code>根据 address 自动判断，值为“域名”或“IP”。</div></div></div><div class="save-row"><button class="button primary" id="saveSettings" type="submit">保存模板与 Token</button><span id="settingsMessage" class="muted" role="status"></span></div></form><div class="api-examples"><div><strong>模板使用样例</strong><small class="muted">以下内容对应名称格式、节点模板和生成结果。</small></div><div class="example-box"><b>节点名称格式</b><code>CF-{{type}}-{{address|split:.:0}}:{{port}}</code></div><div class="example-box"><b>节点模板</b><code>vless://uuid@{{address}}:{{port}}?encryption=none&amp;security=tls#{{name}}</code></div><div class="example-box"><b>生成结果</b><code>vless://uuid@cfsaas.080112.xyz:443?encryption=none&amp;security=tls#CF-%E5%9F%9F%E5%90%8D-cfsaas%3A443</code></div><div class="example-heading"><strong>API 调用案例</strong><small class="muted">每条 URL 单行缩略显示，点击右侧按钮复制完整 URL。</small></div><div class="api-call-row"><span class="method-badge">GET</span><code id="addressExample" title="地址模板调用 URL"></code><button class="button" type="button" data-copy-example="addressExample">复制地址调用 URL</button></div><div class="api-call-row"><span class="method-badge">GET</span><code id="directExample" title="完整节点调用 URL"></code><button class="button" type="button" data-copy-example="directExample">复制完整节点 URL</button></div></div></section><section class="panel nodes-panel"><div class="section-head"><div><h2>API 生成节点</h2><p>按追加顺序展示；只在本页维护，订阅主链接时动态附加在所有主节点后。</p></div><span class="count-badge" id="nodeCount">0 个</span></div><div class="node-list" id="nodeList"></div></section></div></main><dialog id="confirmDialog" class="confirm-dialog" aria-labelledby="confirmTitle"><div class="confirm-body"><h3 id="confirmTitle">请确认操作</h3><p id="confirmText"></p></div><div class="confirm-actions"><button class="button" id="confirmCancel" type="button">取消</button><button class="button primary" id="confirmAccept" type="button">确认</button></div></dialog><script>
		...
		function syncExamples(){var base=window.location.origin+'/api/import?token='+encodeURIComponent(tokenInput.value);document.getElementById('addressExample').textContent=base+'&address=cfsaas.080112.xyz&port=443';document.getElementById('directExample').textContent=base+'&node='+encodeURIComponent('vless://uuid@example.com:443?security=tls#API节点')}
		function render(){document.getElementById('nodeCount').textContent=nodes.length+' 个';if(!nodes.length){list.innerHTML='<div class="empty-list">尚无节点。可通过左侧地址或完整节点 URL 从外部追加。</div>';return}list.innerHTML=nodes.map(function(node){var meta=node.address?(esc(node.address)+(node.port?':'+node.port:'')):'完整节点';return '<article class="node-card"><div class="node-main"><div class="node-head"><span class="node-kind">'+(node.kind==='raw'?'完整节点':'模板生成')+'</span><strong title="'+esc(node.name)+'">'+esc(node.name)+'</strong></div><div class="node-meta">'+meta+' · '+new Date(node.createdAt).toLocaleString()+'</div><div class="node-content" title="'+esc(node.content)+'">'+esc(node.content)+'</div></div><div class="node-actions"><button class="button" type="button" data-copy-node="'+node.id+'">复制</button><button class="button danger-button" type="button" data-delete="'+node.id+'">删除</button></div></article>'}).join('')}
